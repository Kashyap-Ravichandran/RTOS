...
#if USE_ADC_FOR_TOUCHSCREEN
	// Read inputs
		trans.channel=LCD_TS_YU_CHANNEL;
		osMessageQueuePut(id_toADC,&trans,NULL,0);
		fromADC result;
		osStatus_t status1, status2;
		status1=osMessageQueueGet(id_fromADC, &result,NULL,osWaitForever);
		if(result.channel==LCD_TS_YU_CHANNEL&&status1==osOK)
			x=result.value;
		else if(status1==osOK)
			y=result.value;
...
#if USE_ADC_FOR_TOUCHSCREEN
		// Read inputs
		trans.channel=LCD_TS_XL_CHANNEL;
		osMessageQueuePut(id_toADC,&trans,NULL,0);
		//ADC0->SC1[0] = LCD_TS_XL_CHANNEL; // start conversion on channel XL
		//while (!(ADC0->SC1[0] & ADC_SC1_COCO_MASK))
			//;
		//xl = ADC0->R[0];
		status2=osMessageQueueGet(id_fromADC, &result,NULL,osWaitForever);
		if(result.channel==LCD_TS_YU_CHANNEL&&status2==osOK)
			x=result.value;
		else if(status2==osOK)
			y=result.value;
#endif
		//y = xl;
