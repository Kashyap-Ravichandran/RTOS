...
int previous_set_queue[480]={0}, previous_measured_queue[480]={0};
...
void UI_Draw_Fields(UI_FIELD_T * f, int num){
	...
	for (i=0; i < num; i++) {
		if ((f[i].Updated) || (f[i].Volatile)) { // redraw updated or volatile fields
			...
			LCD_Text_Set_Colors(txt_color, bg_color);
...
void UI_Draw_Screen(int first_time) { // Called by Thread_Update_Screen
	...
	//static int queue[480]={0},queue2[480]={0};
	char buffer[32];
		if (first_time) {
		UI_Update_Field_Values(Fields, UI_NUM_FIELDS);
	}
	...
	if(number_elements==480)
		{
			...
			for(int i=0;i<number_elements;i++)
			{
				...
				#if use_refresh
				temp=previous_measured_queue[i];
				temp=100-temp;
				p.X=i;
				p.Y=temp;
				for_line.X=i;
				for_line.Y=100-previous_measured_queue[i+1];
				LCD_Draw_Line(&p,&for_line, &black);
				#endif
				
				temp=queue_Imeasured[i];
				temp=100-temp;
				p.X=i/2;
				p.Y=temp;
				for_line.X=i/2;
				for_line.Y=100-queue_Imeasured[i+1];
				LCD_Draw_Line(&p,&for_line, &yellow);
				//LCD_Plot_Pixel(&p, &yellow);
				
				#if use_refresh
				temp=previous_set_queue[i];
				temp=100-temp;
				p.X=i;
				p.Y=temp;
				for_line.X=i-1;
				for_line.Y=100-previous_set_queue[i+1];
				LCD_Draw_Line(&p,&for_line, &black);
				//LCD_Plot_Pixel(&p, &black);
				#endif
				...
				p.Y=temp;				
				for_line.X=i/2;
				for_line.Y=100-queue_Iset[i+1];
				LCD_Draw_Line(&p,&for_line, &blue);
				//LCD_Plot_Pixel(&p, &blue);
				
				
				previous_measured_queue[i]=queue_Imeasured[i];
				previous_set_queue[i]=queue_Iset[i];
				i++;
