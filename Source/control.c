...
volatile int error;
volatile int previous_set,previous_measured;

volatile int queue_Iset[480]={0}, queue_Imeasured[480]={0};
volatile int front=-1, end=-1, size=0, number_elements;
...
void Control_HBLED(void) {
	...
	// stuff I am gonna add
	if(g_set_current-previous_set>0&&flag==0)
		flag=1;
	
		if(number_elements!=480&&flag==1)
		{	
			queue_Imeasured[number_elements]=g_measured_current;
			queue_Iset[number_elements]=g_set_current;
			number_elements=number_elements+1;
			if(number_elements==480)
			{
				flag=0;
			}
...
void ADC0_IRQHandler() {
	...
	{
		fromADC result;
		result.channel=holdsprev.channel;
		result.value=ADC0->R[0];
		osMessageQueuePut(id_fromADC,&result,1,0);
	}
	// make it software triggered
	...
	int current=TPM0->CNT;
	int a=0;
	a=0;
	a=0;
	int next=TPM0->CNT;
	a= osMessageQueueGetCount(id_toADC);
	if(a&&(current-next>0||(500-next)>50))
	{
		
		osMessageQueueGet(id_toADC, &holdsprev, &attr,0);
		ADC0->SC1[0] &= ~ADC_SC1_ADCH_MASK;
		ADC0->SC1[0]|=holdsprev.channel;
		previous=1;
		// start conversion;
	}
	else
